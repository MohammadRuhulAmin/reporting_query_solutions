...
FROM(SELECT milestone.id M_iD,resuorce.id u_id,
	IFNULL(CONCAT(resuorce.user_first_name,' ',resuorce.user_middle_name,' ',resuorce.user_last_name),'') user_name,
	resuorce.employee_id,p.id project_id,
	CASE WHEN pt.is_commercial=0 THEN 'Non-Commercial' WHEN pt.is_commercial=1 THEN 'Commercial' ELSE 'Production' END project_type,
	YEAR(work_dt) work_year,MONTH(work_dt) work_month,work_dt,team.id team_id,
	team_lead.id team_lead_id,
	IFNULL(CONCAT(team_lead.user_first_name,' ',team_lead.user_middle_name,' ',team_lead.user_last_name),'') Team_lead,
	u.id coordinator_id,
	IFNULL(CONCAT(u.user_first_name,' ',u.user_middle_name,' ',u.user_last_name),'') Coordinator,
	p.ref_no ProjectCode,resuorce.user_email Resource,
	alog.`hour_spent` log_hour,
	REPLACE(REPLACE(REPLACE(REPLACE(alog.`issue_log_details`,'>',''),'-',''),'=',''),'\r\n',' ') Summary 
	FROM project_info p 
