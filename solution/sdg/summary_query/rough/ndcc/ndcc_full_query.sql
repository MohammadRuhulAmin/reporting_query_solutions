SELECT sq1.*,sq2.*,sq3.*,sq4.*,sq5.* FROM(SELECT temp0.ind_id,temp1.sdg_indicator,temp1.sequence_en tier_classification,temp0.agency data_source_md,idss.name activity FROM
...
LEFT JOIN 
(SELECT si.id ind_id,CONCAT(IFNULL(sid.indicator_number,""),' ',IFNULL(sid.short_title,"")) sdg_indicator,
...
ON temp1.ind_id = temp0.ind_id)sq1
LEFT JOIN
...
LEFT JOIN ind_data_source_survey idss ON idss.id = temp2.survey_id)sq2
ON sq1.ind_id = sq2.ind_id
LEFT JOIN 
...
LEFT JOIN ind_data_source_survey idss ON idss.id = temp3.survey_id)sq3
ON sq3.ind_id = sq1.ind_id
LEFT JOIN
...
GROUP BY temp4.ind_id)sq4 ON sq4.ind_id = sq1.ind_id
LEFT JOIN
...
END,", ",DATE_FORMAT(ind_def.last_entry_date, '%M')) AS dead_line
FROM ind_definitions ind_def)sq5 ON sq5.ind_id = sq1.ind_id
WHERE sq1.data_source_md != "Bangladesh Bureau of Statistics"