...
SELECT * FROM indicator_data WHERE ind_id = 123 AND source_id IN (82,63,187);


SELECT merge_record.indicator_number,merge_record.ind_id,GROUP_CONCAT( merge_record.data_status SEPARATOR '/') data_status,
GROUP_CONCAT(DISTINCT merge_record.due_data SEPARATOR '/') due_data ,merge_record.ind_src_id,merge_record.office_agency_id,merge_record.agency
FROM(SELECT temp_info.*,agency_info.*
FROM(SELECT sid.indicator_number,temp1.ind_id,temp1.source_id,CASE WHEN temp1.curr_report_year = temp1.max_data_period THEN "updated"
...
LEFT JOIN ind_sources ind_src ON ind_src.id = temp1.source_id)temp_info
LEFT JOIN
(SELECT ind_sources.id ind_src_id, office_agency_id,office_agencies.name agency FROM ind_sources 
LEFT JOIN office_agencies ON office_agencies.id = ind_sources.office_agency_id 
WHERE office_agency_id > 0 #GROUP BY office_agency_id
...
ON temp_info.source_id = agency_info.ind_src_id)merge_record
WHERE merge_record.indicator_number = "9.5.2"
GROUP BY merge_record.office_agency_id;