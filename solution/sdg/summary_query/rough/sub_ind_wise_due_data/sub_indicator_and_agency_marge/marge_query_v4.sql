...
FROM(SELECT sid2.indicator_number prnt_indicator_number,tbl_agency.agency, pind_info.parent_indicator_id,sid.indicator_number,
#ind_src.name,
...
ON tempz.ind_id = tempx.ind_id #WHERE tempx.ind_id IN(249,251) 
...
INNER JOIN
(SELECT id,parent_indicator_id FROM sdg_indicators WHERE parent_indicator_id > -1)pind_info ON pind_info.id = temp1.ind_id
...
WHERE sq2.prnt_indicator_number  IN(SELECT temp2.indicator_number FROM(SELECT sid.indicator_number,temp1.ind_id, GROUP_CONCAT(DISTINCT temp1.ind_src_id) source_list,
...
SELECT sid.indicator_number,tbl_agency.agency, #pind_info.parent_indicator_id,
#temp1.ind_id,temp1.source_id,
...
INNER JOIN
(SELECT id,parent_indicator_id FROM sdg_indicators WHERE parent_indicator_id > -1)pind_info ON pind_info.id = temp1.ind_id
LEFT JOIN sdg_indicator_details sid2 ON sid2.indicator_id = pind_info.parent_indicator_id AND sid2.language_id = 1
