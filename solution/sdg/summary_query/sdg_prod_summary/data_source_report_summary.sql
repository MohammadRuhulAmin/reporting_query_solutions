...
SELECT parcentage.agency,FORMAT((100/parcentage.total_indicator)*parcentage.total_updated,2) total_update_parcent, parcentage.total_updated,
...
parcentage.total_indicator
FROM(SELECT SUM(CASE WHEN summary.data_status = "updated" THEN 1 ELSE 0 END) total_updated,
SUM(CASE WHEN summary.data_status = "not updated" THEN 1 ELSE 0 END)AS total_not_updated,
SUM(CASE WHEN summary.data_status = "no data" THEN 1 ELSE 0 END) AS total_no_base_line,
summary.agency,
"100" AS total_indicator_parcent,COUNT(summary.indicator_number) total_indicator 
FROM(SELECT sx1.agency,target_info.goal_id,target_info.target_number,sx1.prnt_indicator_number indicator_number,
...
GROUP BY summary.agency)parcentage
GROUP BY parcentage.agency;
