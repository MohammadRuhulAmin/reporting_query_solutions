SELECT sg.order goal_number,temp.* FROM(SELECT iawrd.* FROM(SELECT tempm.*,templ.* FROM
(SELECT sx4.institution,sx4.type,sx4.agency_id ,sx1.*,sx2.last_reporting_year,sx3.next_reporting_year 
...
WHEN sq2.total_no_base_line = sq2.total_records THEN "no data"
ELSE "not updated" END AS data_status,sq2.due_data   FROM
(SELECT sq1.prnt_indicator_number,sq1.indicator_id,sq1.agency,
GROUP_CONCAT( CONCAT(sq1.indicator_number , "(",sq1.source_id),")") source_list,
SUM(CASE WHEN sq1.data_status = "updated" THEN 1 ELSE 0 END) total_updated,
SUM(CASE WHEN sq1.data_status = "not updated" THEN 1 ELSE 0 END) total_not_update,
SUM(CASE WHEN sq1.data_status = "no data" THEN 1 ELSE 0 END) total_no_base_line,
COUNT(sq1.prnt_indicator_number) total_records,
GROUP_CONCAT(CONCAT(sq1.indicator_number , "(",sq1.due_data),")") due_data
FROM(SELECT sid2.indicator_number prnt_indicator_number,sid2.indicator_id,tbl_agency.office_agency_id,tbl_agency.agency, pind_info.parent_indicator_id,sid.indicator_number,
#ind_src.name,
temp1.ind_id,temp1.source_id,CASE WHEN temp1.curr_report_year = temp1.max_data_period THEN "updated"
WHEN temp1.total_next_report_year <> temp1.missing_data_year  THEN "not updated"
WHEN temp1.total_next_report_year = temp1.missing_data_year  THEN "no data"
...
END AS data_status,temp1.missing_data_year_list due_data
FROM 
(SELECT temp0.ind_id,temp0.agency,temp0.office_agency_id institution_id,GROUP_CONCAT(DISTINCT temp0.source_id) source_id_list,
...
GROUP BY si.id)sx2 ON sx2.ind_id = sx1.indicator_id
LEFT JOIN
(SELECT ind_id,base_data_period +
(data_frequency_year*CEIL((YEAR(NOW())-base_data_period)/data_frequency_year)) next_reporting_year
FROM ind_definitions WHERE base_data_period <> 0000)sx3 ON sx3.ind_id = sx1.indicator_id
LEFT JOIN
(SELECT idm.id ind_src_id,idm.ind_id, idm.type,idm.office_agency_id agency_id,office_agencies.name institution 
...
WHERE office_agency_id = 0 AND ministry_division_id = 0 AND ministry_id >0)sx4 ON sx4.ind_id = sx1.indicator_id

ORDER BY sx1.indicator_id)templ
LEFT JOIN

(SELECT si.id,si.goal_id,si.target_id,sdg_td.target_number FROM sdg_indicators si
LEFT JOIN sdg_target_details sdg_td ON sdg_td.target_id = si.target_id
WHERE sdg_td.language_id = 1)tempm ON tempm.id = templ.indicator_id
ORDER BY templ.indicator_id)iawrd)temp
LEFT JOIN sdg_goals sg ON sg.id = temp.goal_id
