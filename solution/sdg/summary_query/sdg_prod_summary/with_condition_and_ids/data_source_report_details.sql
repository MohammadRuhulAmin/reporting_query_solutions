SELECT solution.* FROM (SELECT sidj.indicator_id,stdj.target_id,about_agency.office_agency_id,
joiner.* FROM (SELECT sx1.agency,target_info.goal_id,target_info.target_number,sx1.prnt_indicator_number indicator_number,
...
ORDER BY sx1.indicator_id)joiner 
LEFT JOIN sdg_indicator_details sidj  ON joiner.indicator_number = sidj.indicator_number
LEFT JOIN sdg_target_details stdj ON joiner.target_number = stdj.target_number
LEFT JOIN (SELECT ind_sources.id ind_src_id, office_agency_id,office_agencies.name agency FROM ind_sources 
LEFT JOIN office_agencies ON office_agencies.id = ind_sources.office_agency_id 
WHERE office_agency_id > 0 
UNION 
SELECT ind_sources.id, ministry_division_id,ministry_divisions.name FROM ind_sources 
...
SELECT ind_sources.id, survey_id,ind_data_source_survey.name FROM ind_sources 
LEFT JOIN ind_data_source_survey ON ind_data_source_survey.id = ind_sources.survey_id 
WHERE office_agency_id = 0 AND ministry_division_id = 0 AND ministry_id =0 AND survey_id >0) about_agency
ON about_agency. agency = joiner.agency

WHERE sidj.language_id = 1 AND stdj.language_id = 1

GROUP BY sidj.indicator_id, stdj.target_id, joiner.agency, joiner.goal_id,joiner.target_number,joiner.indicator_number,
joiner.data_status,joiner.due_data,joiner.last_reporting_year,joiner.next_reporting_year)solution
WHERE 1 = 1;