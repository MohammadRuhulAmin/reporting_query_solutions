SELECT sil.serial_no,temp_marge.indicator_id,temp_marge.provider_id,temp_marge.status,temp_marge.id,temp_marge.user_id,temp_marge.created_by,
...
FROM (SELECT temp_sidp.provider_id,temp_sidp.indicator_id,temp_sidp.status,sp.id,sp.user_id,sp.created_by,
...
LEFT JOIN `sdg_v1_v2_live`.`sdg_indicator_langs` sil ON sil.id = temp_marge.indicator_id AND sil.language_id = 1
