...
(SELECT pk_idd.`ind_data_id` park_ind_data_id, pk_idd.`disagg_id`,pk_idd.`disagg_name` disagg_name, pk_dn.id, pk_dn.name,pk_dn.type_id type_id
...
WHERE pk_idd.ind_data_id = 2 ORDER BY pk_idd.disagg_id)temp_parking
...
(SELECT prd_idd.`ind_data_id` prod_ind_data_id,prd_idd.disagg_id,prd_idd.disagg_name disagg_name, prd_dn.id,prd_dn.name,prd_dn.type_id type_id
FROM `sdg-prod`.indicator_disagg_data prd_idd
LEFT JOIN `sdg-prod`.disaggregation_name prd_dn ON prd_dn.id = prd_idd.disagg_id
WHERE prd_idd.ind_data_id = 2 ORDER BY prd_idd.disagg_id)temp_prod
